...
from yolo3.models import yolo_eval, yolo_body, tiny_yolo_body,yolo_head
from yolo3.util import letterbox_image
import os
from cv2 import cv2
...
class YOLO(object):
    ...
    _defaults = { #函数默认参数
        #模型路径
        # "model_path": 'logs/006/trained_weights_final.h5',
        #锚点路径
    ...
    def __init__(self, **kwargs):
        ...
        self.model_path = 'logs/034/trained_weights_final.h5'
        #锚点路径
        self.anchors_path = 'anchor4.txt'
        #种类路径
    ...
    def generate(self):
        ...
        try:
            ...
            self.yolo_model = tiny_yolo_body(Input(shape=(None,None,3)), num_anchors//2, num_classes) \
                if is_tiny_version else yolo_body(Input(shape=(None,None,3)), num_anchors//4, num_classes)
            #调用完模型调用权重
            self.yolo_model.load_weights(self.model_path) # make sure model, anchors and classes match
        else:#锚点个数除以（模型输出乘以种类个数加5）
            # print("anchors")
            # print('{} model, anchors, and classes loaded.'.format(model_path))
            # print(self.yolo_model.layers[-1].output_shape[-1]) -->255
            # print(num_anchors) -->9
            # print(len(self.yolo_model.output) ) --->3
...
#重点
    ...
    def detect_image(self, image):
        ...
        #将图片转换成numpy数据处理
        boxed_image = boxed_image.convert('RGB')
        ...
        for i, c in reversed(list(enumerate(out_classes))):
            ...
            print(label, (left, top), (right, bottom),label_size[1])
            ...
            #del语句作用在变量上，而不是数据对象上，del删除的是变量，而不是数据。释放draw这个变量
            del draw
...
def main():
    ...
    r_image.save("logs/034/000004.jpg")
    r_image.show()
